...
// set the correct BUAD and F_CPU defines before including setbaud.h
...
//buffers for use with ring buffer
uint8_t out_buffer[UART_BUFFER_SIZE];
uint8_t in_buffer[UART_BUFFER_SIZE];

//! ring buffer to use for UART transmission
struct ring_buffer ring_buffer_out;

//! ring buffer to use for UART reception
...
*
* This is the handle for the UART receive data
*/
ISR(UART0_RX_IRQ)
{
...
* values using the AVR libc setbaud utility. In additin set the UART
...
#error "Device is not supported by the driver"
	...
	ring_buffer_out = ring_buffer_init(out_buffer, UART_BUFFER_SIZE);
	ring_buffer_in = ring_buffer_init(in_buffer, UART_BUFFER_SIZE);
}
...
/**
* \brief Function for getting a char from the UART recieve buffer
*
